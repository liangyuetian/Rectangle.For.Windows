...
public class AcrylicMenuRenderer : ToolStripProfessionalRenderer
{
    ...
    private static readonly Color SelectedColor = Color.FromArgb(255, 0, 120, 212);
    ...
    {
        var item = e.Item;
        e.TextColor = item.Enabled ? TextColor : DisabledColor;
        
        if (item is ToolStripMenuItem menuItem && !string.IsNullOrEmpty(menuItem.ShortcutKeyDisplayString))
        {
            // 绘制主文本
            var textRect = new Rectangle(e.TextRectangle.X, e.TextRectangle.Y, 
                e.TextRectangle.Width, e.TextRectangle.Height);
            TextRenderer.DrawText(e.Graphics, e.Text, e.TextFont, textRect, 
                e.TextColor, TextFormatFlags.Left | TextFormatFlags.VerticalCenter);

            // 绘制快捷键文本（右对齐，灰色）
            var shortcutRect = new Rectangle(item.Width - 150, e.TextRectangle.Y, 
                140, e.TextRectangle.Height);
            TextRenderer.DrawText(e.Graphics, menuItem.ShortcutKeyDisplayString, e.TextFont, 
                shortcutRect, ShortcutColor, TextFormatFlags.Right | TextFormatFlags.VerticalCenter);
        }
        else
        {

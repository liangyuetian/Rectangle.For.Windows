...
namespace Rectangle.Windows.WinUI.Services
{
    ...
    public class LayoutManager
    {
        ...
        private string GetWindowTitle(nint hwnd)
        {
            try
            {
                var length = Windows.Win32.PInvoke.GetWindowTextLength(new Windows.Win32.Foundation.HWND(hwnd));
                if (length == 0) return string.Empty;

                unsafe
                {
                    char* buffer = stackalloc char[length + 1];
                    Windows.Win32.PInvoke.GetWindowText(new Windows.Win32.Foundation.HWND(hwnd), buffer, length + 1);

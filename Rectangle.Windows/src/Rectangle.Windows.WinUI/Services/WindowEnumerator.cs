...
namespace Rectangle.Windows.WinUI.Services
{
    ...
    public static class WindowEnumerator
    {
        ...
        /// 枚举所有可见窗口
        /// </summary>
        public static unsafe List<nint> EnumerateVisibleWindows(bool excludeMinimized = true, bool excludeToolWindows = true)
        {
            var windows = new List<nint>();

            PInvoke.EnumWindows((hwnd, lParam) =>
            {
                var h = (nint)hwnd.Value;
                if (IsWindowValid(h, excludeMinimized, excludeToolWindows))
                {
                    windows.Add(h);
                }
                return true;
        ...
        /// 枚举指定进程的所有窗口
        /// </summary>
        public static unsafe List<nint> EnumerateWindowsByProcess(string processName, bool excludeMinimized = true)
        {
            var windows = new List<nint>();
            ...
            {
                var h = (nint)hwnd.Value;
                if (IsWindowValid(h, excludeMinimized, true))
                {
                    var process = GetProcessNameFromWindow(h);

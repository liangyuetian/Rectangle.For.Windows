    ...
    public class OperationHistoryManager
    {
            ...
            WindowRect rectAfter)
        {
            lock (_lock)
            {
                ...
                Logger.Debug("OperationHistory", $"[OperationHistory] 记录操作: {item.Description}");
        ...
        public OperationHistoryItem? Undo()
        {
            lock (_lock)
            {
                ...
                if (item == null)
                {
                    Logger.Debug("OperationHistory", "[OperationHistory] 没有可撤销的操作");
                    return null;
                }

                item.IsUndone = true;
                Logger.Info("OperationHistory", $"[OperationHistory] 撤销操作: {item.Description}");
        ...
        public OperationHistoryItem? Redo()
        {
            lock (_lock)
            {
                ...
                if (item == null)
                {
                    Logger.Debug("OperationHistory", "[OperationHistory] 没有可重做的操作");
                    return null;
                }

                item.IsUndone = false;
                Logger.Info("OperationHistory", $"[OperationHistory] 重做操作: {item.Description}");
        ...
        public void ClearHistory()
        {
            ...
            {
                _history.Clear();
                Logger.Info("OperationHistory", "[OperationHistory] 历史记录已清空");
        ...
        public void RemoveHistoryForWindow(nint windowHandle)
        {
            lock (_lock)
            {
                ...
                Logger.Debug("OperationHistory", $"[OperationHistory] 移除窗口 {windowHandle} 的历史记录");
            }
        }

...
namespace Rectangle.Windows.WinUI.Services
{
    ...
    public class EdgeIndicatorConfig
    {
        ...
        /// 正常状态颜色
        /// </summary>
        public Windows.UI.Color NormalColor { get; set; } = Windows.UI.Color.FromArgb(80, 0, 120, 215);

        /// <summary>
        /// 悬停状态颜色
        /// </summary>
        public Windows.UI.Color HoverColor { get; set; } = Windows.UI.Color.FromArgb(180, 0, 150, 255);

        /// <summary>
        /// 激活状态颜色（拖拽时）
        /// </summary>
        public Windows.UI.Color ActiveColor { get; set; } = Windows.UI.Color.FromArgb(255, 0, 180, 255);
    ...
    public class EdgeIndicatorWindow : Window
    {
        ...
        private void InitializeWindow()
        {
            ...
            if (hwnd != 0)
            {
                ...
                exStyle |= (int)WINDOW_EX_STYLE.WS_EX_TOPMOST;
                ...
                PInvoke.SetWindowLong((HWND)hwnd, WINDOW_LONG_PTR_INDEX.GWL_EXSTYLE, exStyle);
            }
        }
    ...
    public class EdgeIndicatorService
    {
        ...
        public void Initialize()
        {
            if (!_config.Enabled)
            {
                _logger?.LogDebug("[EdgeIndicator] 边缘指示器已禁用");
                return;
            }
            ...
            _logger?.LogInfo("[EdgeIndicator] 边缘指示器服务已初始化");
        }
        ...
        private void CreateIndicatorsForPrimaryScreen()
        {
            // 注意：实际实现需要在UI线程上创建窗口
            // 这里使用简化版本
            _logger?.LogDebug("[EdgeIndicator] 创建边缘指示器窗口");
        }

        /// <summary>
        /// 鼠标移动事件处理
        /// </summary>
        private void OnMouseMove(object? sender, MouseHookService.MouseEventArgs e)
        {
        ...
        /// 鼠标按下事件处理
        /// </summary>
        private void OnMouseDown(object? sender, MouseHookService.MouseEventArgs e)
        {
            _isDragging = true;
        ...
        /// 鼠标释放事件处理
        /// </summary>
        private void OnMouseUp(object? sender, MouseHookService.MouseEventArgs e)
        {
            _isDragging = false;
        ...
        private void CheckEdgeProximity(int mouseX, int mouseY)
        {
            ...
            foreach (var screen in screens)
            {
                ...
                // 检查各边缘
                bool nearLeft = mouseX >= bounds.Left && mouseX <= bounds.Left + _config.TriggerDistance;
                bool nearRight = mouseX >= bounds.Right - _config.TriggerDistance && mouseX <= bounds.Right;
                bool nearTop = mouseY >= bounds.Top && mouseY <= bounds.Top + _config.TriggerDistance;
                bool nearBottom = mouseY >= bounds.Bottom - _config.TriggerDistance && mouseY <= bounds.Bottom;
        ...
        public void ShowIndicators()
        {
            ...
            _logger?.LogDebug("[EdgeIndicator] 显示边缘指示器");
        }
        ...
        public void HideIndicators()
        {
            ...
            _logger?.LogDebug("[EdgeIndicator] 隐藏边缘指示器");
        }

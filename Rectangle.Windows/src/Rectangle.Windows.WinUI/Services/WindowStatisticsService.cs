...
namespace Rectangle.Windows.WinUI.Services
{
    ...
    public class WindowStatisticsService
    {
        ...
        public WindowStatisticsService(Logger logger)
        {
            _logger = logger;
        ...
        public void RecordWindowActivation(nint hwnd, string windowClass, string applicationName, string executableName)
        {
            ...
            lock (_lock)
            {
                ...
                _logger?.LogDebug($"[Statistics] 窗口激活: {applicationName}");
            ...
            double executionTimeMs = 0)
        {
            ...
            lock (_lock)
            {
                ...
                if (_heatmapData.Count < MaxHeatmapPoints)
                {
                    ...
                    {
                        X = (double)rect.X / SystemParameters.PrimaryScreenWidth,
                        Y = (double)rect.Y / SystemParameters.PrimaryScreenHeight,
                        Intensity = 1.0,
                        Action = action,
        ...
        public async Task SaveStatisticsAsync()
        {
            try
            {
                ...
                _logger?.LogInfo($"[Statistics] 统计数据已保存: {filePath}");
            }
            catch (Exception ex)
            {
                _logger?.LogError($"[Statistics] 保存统计数据失败: {ex.Message}");
            }
        }
        ...
        public async Task LoadStatisticsAsync()
        {
            try
            {
                ...
                if (data != null)
                {
                    ...
                    _logger?.LogInfo($"[Statistics] 已加载历史统计数据");
                }
            }
            catch (Exception ex)
            {
                _logger?.LogError($"[Statistics] 加载统计数据失败: {ex.Message}");
            }
        }
        ...
        public void ClearStatistics()
        {
            ...
            _logger?.LogInfo("[Statistics] 统计数据已清除");
            StatisticsUpdated?.Invoke(this, EventArgs.Empty);
        }
        ...
        public void CleanupOldData()
        {
            ...
            try
            {
                ...
                foreach (var file in files)
                {
                    ...
                    {
                        File.Delete(file);
                        _logger?.LogDebug($"[Statistics] 删除过期统计文件: {file}");
                    }
                }
            }
            catch (Exception ex)
            {
                _logger?.LogError($"[Statistics] 清理过期数据失败: {ex.Message}");
            }
        }
        ...
        public async Task<string> ExportStatisticsAsync(string format = "json")
        {
            ...
            _logger?.LogInfo($"[Statistics] 统计数据已导出: {filePath}");
            return filePath;
        }
    }
